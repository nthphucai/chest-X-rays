...
def get_adjacency_matrix(smooth_corr, neighbor_ratio=0.2):
    ...
    reweight = (
        ...
        * neighbor_ratio
        / (1 - neighbor_ratio)
        / (reweight.values.sum(axis=0, keepdims=True) + 1e-8)
    )
    ...
    return pd.DataFrame(
        normalized, index=smooth_corr.index, columns=smooth_corr.columns
    )
        ...
        for c2 in corr.columns:
            if c2 != c1:
                ...
                if smooth_corr.loc[c1, c2] != 0:
                    edge = pydot.Edge(
                        node1, node2, label=np.round(corr.loc[c1, c2], decimals=2)
                    )
                    graph.add_edge(edge)
