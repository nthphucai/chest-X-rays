...
class GraphLinear(nn.Module):
    def __init__(self, in_channels, out_channels, adj_mtrix, bias=False):
        ...
        self.adj_mtrix = nn.Parameter(
            torch.tensor(adj_mtrix, dtype=torch.float), requires_grad=False
        )
        if bias:
...
class GraphSequential(nn.Module):
    ...
    def forward(self):
        ...
        )  # self.embedding_notes là input của self.sequential()


class GCN(nn.Module):
    def __init__(
        self, in_channels, out_channels, embedding_nodes, adj_mtrix, bias=True
    ):
        super().__init__()

        bottleneck = int(np.round(out_channels // 2))
        self.embedding = GraphSequential(
            embedding_nodes,
            *[
