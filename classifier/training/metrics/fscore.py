...
class FbetaScore(nn.Module):
    ...
    def forward(self, preds, trues):
        ...
        if self.classes is not None:
            if len(fb.shape) == 1:
                fb = fb[np.newaxis]
            results = {
                f"{c}_F{self.beta}": fb[:, i].mean() for i, c in enumerate(self.classes)
            }
    ...
    def extra_repr(self):
        return (
            f"beta={self.beta}, axes={self.axes}, binary={self.binary}, "
...
class AUCScore(nn.Module):
    def __init__(self, binary=True, classes=None):
        ...
        if ".csv" in classes:
            ...
            self.classes = np.load(classes, allow_pickle=True)
    ...
    def forward(self, preds, trues):
        ...
        results = {f"{c}": auc_score[i] for i, c in enumerate(self.classes)}
