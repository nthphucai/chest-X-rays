...
class Dice_3D:
    ...
    def update_class(self, probs: torch.Tensor, targets: torch.Tensor):
      ...
      """
      dsc_organ, dsc_tumor  = self.dice_class(probs.detach().cpu().numpy(), targets.detach().cpu().numpy(), self.threshold)
    ...
    def update_batch(self, probs: torch.Tensor, labels: torch.Tensor):  
      ...
      dice = self.dice_batch(probs.detach().cpu().numpy(), labels.detach().cpu().numpy(), threshold = self.threshold)
    ...
    def dice_class(probs: np.ndarray,
                  ...
                  threshold: float = 0.5,
                  eps: float = 1e-9,
                  classes: list = ['organ', 'tumor']) -> np.ndarray:
