    ...
    def _train_one_epoch(self, epoch: int, callbacks):
      ...
      with get_progress(total=len(self.dl_train)) as pbar:
        for step, batch_data in enumerate(self.dl_train):
            ...
            pbar.update()
            [c.on_training_batch_end(epoch=epoch, step=step, data=data, logs=total_loss) for c in callbacks]
        ...
        with torch.no_grad():
            with get_progress(total=len(self.dl_val)) as pbar:
                for step, batch_data in enumerate(self.dl_train):
                    ...
                    [c.on_validation_batch_begin(epoch, step=step, data=data) for c in callbacks]
                    loss, preds   = self._eval_one_batch(imgs, targets)
                    total_loss += loss.item()
                    subscore   += self._measures_one_batch(preds, targets)

                    pbar.update()
                    [c.on_validation_batch_end(epoch=epoch, step=step, data=data, logs=total_loss) for c in callbacks]
