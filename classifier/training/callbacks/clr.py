...
class LrFinder(TrainerCallback):
    ...
    def plot_data(self, group=0, target="loss"):
        ...
        if self.use_plotly:
            ...
            fig.update_layout(
                title=f"lr vs {target}", xaxis_title="lr", yaxis_title=target
            )
            fig.show("iframe")
        else:
...
class WarmRestart(TrainerCallback):
    ...
    def on_train_begin(self, **train_configs):
        ...
        self.scheduler = opts.lr_scheduler.CosineAnnealingWarmRestarts(
            self.optimizers[0],
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.run_epoch == self.max_epoch:
            ...
            print(
                "starting cycle ", self.finished_cycles + 1
            ) if self.finished_cycles != self.cycles else None
            if self.snapshot:
                ...
                torch.save(
                    model_dict,
                    f"{self.dir}/snapshot_{self.model_name}_cycle_{self.finished_cycles}.model",
                )
                torch.save(
                    optim_dict,
                    f"{self.dir}/snapshot_{self.model_name}_cycle_{self.finished_cycles}.optim",
                )
    ...
    def extra_repr(self):
        return (
            f"min_lr={self.min_lr}, t0={self.t0}, factor={self.factor}, cycles={self.cycles}, "
...
class SuperConvergence(TrainerCallback):
    ...
    def on_train_begin(self, **train_configs):
        ...
        self.scheduler = opts.lr_scheduler.OneCycleLR(
            self.optimizers[0],
            max_lr,
            epochs=self.epochs,
            steps_per_epoch=n,
    ...
    def extra_repr(self):
        return (
            f"epochs={self.epochs}, div_factor={self.div_factor}, final_div_factor={self.final_div_factor}, pct_start={self.pct_start}, "
