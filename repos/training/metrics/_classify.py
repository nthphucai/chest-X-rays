...
class F1Score(nn.Module):
    ...
    def forward(self, preds, trues):
        ...
        fb = torch.tensor([f1_score(trues[:, i], preds[:, i], average='binary', zero_division=1, **self.kwargs) for i in range(trues.shape[1])])
...
class prec_recall_fscore_support(nn.Module):
    ...
    def __init__(self, beta=1, binary=True, classes=None, metric='Fscore', average='binary', **kwargs):
        super().__init__()
    ...
    def forward(self, preds, trues):
        ...
        results = [pcfs(trues[:, i], preds[:, i], average='binary', zero_division=1, **self.kwargs) for i in range(trues.shape[1])]
        if self.metric == 'Fscore':
          result  = torch.Tensor([results[i][mtric['Fscore']] for i in range(trues.shape[1])])
        elif self.metric == 'Precision':
           result = torch.Tensor([results[i][mtric['Precision']] for i in range(trues.shape[1])])
        elif self.metric == 'Recall':
           result = torch.Tensor([results[i][mtric['Recall']] for i in range(trues.shape[1])])

        if self.classes is not None:
            result = {f"{c}_{self.metric}": result[i] for i, c in enumerate(self.classes)}
            return result
        
        return {f"{self.metric}": result.mean()}

    def extra_repr(self):
        return f"metric={self.metric}, binary={self.binary}, " \
               f"classes={self.classes}"
